...
export function CreateShippingModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  const handleCreate = async () => {
    if (!name || !price || !deliveryTime) {
      toast.error(intl.formatMessage({ id: 'modal.shipping.fillAll' }));
      return;
    }
    ...
    try {
      ...
      toast.success(intl.formatMessage({ id: 'modal.shipping.createSuccess' }));
      onClose();
      onSuccess();
    } catch (err) {
      console.error(err);
      toast.error(intl.formatMessage({ id: 'modal.shipping.createError' }));
    }
  };
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className={`${isDarkMode ? 'bg-[var(--card-background)] border-white/5' : 'bg-white border-gray-200'} rounded-xl w-full max-w-md p-6 border`}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold">{intl.formatMessage({ id: 'modal.shipping.new' })}</h2>
        ...
        <div className="space-y-4">
          <label className="block">
            ...
            <input
              ...
              placeholder={intl.formatMessage({ id: 'modal.shipping.namePlaceholder' })}
            />
          </label>
          ...
          <label className="block">
            ...
            <input
              ...
              className={`mt-1 w-full rounded-lg border-2 px-3 py-2 text-sm ${isDarkMode ? 'bg-[var(--card-background)] border-white/5' : 'bg-gray-50 border-gray-200'} focus:border-[var(--primary-color)]`}
              placeholder={intl.formatMessage({ id: 'modal.shipping.pricePlaceholder' })}
            />
          </label>
        ...
        <div className="mt-6 flex justify-end">
          ...
          >
            {intl.formatMessage({ id: 'modal.shipping.create' })}
          </button>
        </div>
      </div>
    </div>
